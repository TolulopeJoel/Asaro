/**
 * Spacing, radius and elevation. Both styles are flat and square: Cloth's edges
 * come from colour blocks and hairlines, so the radius set is deliberately
 * tiny.
 */
...
export const Spacing = {
    ...
    /**
     * The design has exactly two radii: square, and round for avatars. Every
     * panel, cell, input and button in design/all-screens.html is
     * `border-radius:0`; the only curve in the mockup is
     * `.cl-avatar{border-radius:50%}`.
     *
     * `sm`/`md`/`lg`/`xl` are aliases of `none` so existing call sites land on
     * the real system. New code should ask the theme — `shape.card`,
     * `shape.button`, `shape.input`, `shape.chip` — which is the axis a future
     * style can move.
     */
    borderRadius: {
        none: 0,
...
export const Motif = {
    ...
    rings: {
        ...
        radius: 4.5,
        /**
         * Radial repeat interval. The mockup's motif is a
         * repeating-radial-gradient restarting every 12px, so rings land at
         * 4.5, 16.5 and 28.5 — derive them from this, never an eyeballed offset.
         */
        step: 12,
