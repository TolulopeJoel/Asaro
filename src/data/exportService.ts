                ...
                if (entry.action_items && entry.action_items.length > 0) {
                    ...
                    for (let i = 0; i < entry.action_items.length; i++) {
                        ...
                        await database.runAsync(
                            ...
                            `INSERT INTO action_items (entry_id, action, motivation, sort_order, is_completed, is_pinned, cadence, due_at, archived_at)
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                            [newEntryId, item.action ?? '', item.motivation ?? '', item.sort_order ?? i, item.is_completed ? 1 : 0, item.is_pinned ? 1 : 0, item.cadence ?? null, item.due_at ?? null, item.archived_at ?? null]
                        );
                    }
                }
